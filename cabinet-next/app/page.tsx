...
import {
  ...
  MessageSquareText, MoreHorizontal, MousePointerClick, Pipette, Plus, Rocket, Search, Send,
...
function Dashboard({ setView, onAction, analytics, period, onPeriodChange, customFrom, customTo, onCustomRange, readiness, activity }: { setView: (v: View) => void; onAction: (label: string) => void; analytics: CabinetAnalytics; period: AnalyticsPeriod; onPeriodChange: (p: AnalyticsPeriod) => void; customFrom: string | null; customTo: string | null; onCustomRange: (from: string, to: string) => void; readiness: ReadinessData | null; activity: ActivityData | null }) {
  ...
  // This card used to show a hardcoded 75% ring regardless of the real bot
  // (found live: disagreed with the sidebar's "Внедрение идёт", which reads
  // the actual readiness.percent) — same ring math as ReadinessRing, driven
  // by the same readiness prop everything else on this page already uses.
  const readinessPercent = readiness?.percent;
  const RING_R = 37;
  const RING_C = 2 * Math.PI * RING_R;
  const readinessOffset = RING_C * (1 - (readinessPercent ?? 0) / 100);
  return <>
    ...
    <section className="dashboard-grid">
      ...
      <ConversionChart/>
      <article className="panel readiness-card"><div className="readiness-ring"><svg viewBox="0 0 88 88"><circle cx="44" cy="44" r="37" /><circle className="ready" cx="44" cy="44" r="37" style={{ strokeDasharray: RING_C, strokeDashoffset: readinessOffset }} /></svg><strong>{readinessPercent === undefined ? "…" : `${readinessPercent}%`}</strong></div><div><span className="section-label">Внедрение с менеджером</span><h2>Подготовка к запуску</h2><p>Менеджер настраивает сценарий, знания и подключения. Здесь виден общий статус.</p><button className="inline-action" data-live onClick={() => setView("readiness")}>Открыть план <ArrowRight /></button></div></article>
      {(() => {
...
function Readiness({ setView, readiness }: { setView: (v: View) => void; readiness: ReadinessData | null }) {
  ...
  // ImplementationManagerService's own comment on why per-company
  // assignment isn't built yet.
  const [manager, setManager] = useState<{ name: string; photoUrl: string | null } | null>(null);
...
function Training({ me, activeBotId }: { me: CabinetMe; activeBotId: string | null }) {
  ...
  // Neither was ever forwarded to this preview before (found live: no way to
  // actually see "Расположение: слева" anywhere — only a real site re-
  // copying the embed snippet ever reflected it). Own small fetch rather
  // than lifting WidgetSettings' state up: the two panes are never mounted
  // at the same time (different nav items), so there's nothing to keep in
  // sync between them.
  const [appearance, setAppearance] = useState<{ color: string; position: string } | null>(null);
  useEffect(() => {
    setAppearance(null);
    fetchJsonWithRetry<{ color: string; position: string }>(`/api/cabinet/appearance${activeBotId ? `?botId=${activeBotId}` : ""}`).then((data) => {
  ...
  const previewSrc = token
    ? `/cabinet/test-widget-preview.html?token=${encodeURIComponent(token)}&autoopen=1` +
      (appearance ? `&color=${encodeURIComponent(appearance.color)}&position=${encodeURIComponent(appearance.position)}` : "")
    : undefined;
  const copyLink = () => {
...
function WidgetSettings({ me, setMe, activeBotId, analytics, refetchAnalytics }: { me: CabinetMe; setMe: React.Dispatch<React.SetStateAction<CabinetMe>>; activeBotId: string | null; analytics: CabinetAnalytics; refetchAnalytics: () => void }) {
  ...
  const generateAvatar = () => {
    setAvatarBusy(true);
    setAvatarError(null);
    // Sends whatever's picked in the form right now, not just what's already
    // saved — switching "Голос" to "Мужской" and generating without hitting
    // "Сохранить изменения" first must still produce a male photo (found
    // live: got a female one, because the backend used to only ever read the
    // already-persisted gender).
    fetch(`/api/cabinet/appearance/avatar/generate${botQuery}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ gender: form?.gender }),
    })
      .then((r) => (r.ok ? r.json() : r.json().catch(() => null).then((b) => Promise.reject(new Error(b?.message)))))
      .then((data: Appearance) => setForm((f) => f && { ...f, avatarUrl: data.avatarUrl }))
...
// Same simple perceptual-luminance heuristic the real widget uses (see
// chat.js's own colorParam handling) — picked here too so the cabinet's live
// preview shows exactly what a visitor will actually see, including for a
// color picked with the eyedropper straight off someone else's dark site
// (found live: white-on-dark now, but the header text stayed dark-on-dark
// until this).
function contrastTextColor(hex: string | undefined): string {
  if (!hex || !/^#[0-9a-fA-F]{6}$/.test(hex)) return "#081522";
  const r = parseInt(hex.slice(1, 3), 16);
  const g = parseInt(hex.slice(3, 5), 16);
  const b = parseInt(hex.slice(5, 7), 16);
  const luminance = 0.299 * r + 0.587 * g + 0.114 * b;
  return luminance < 140 ? "#fff" : "#081522";
}
