...
export default function Home() {
  ...
  const handleFallback = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (button.closest(".app-sidebar,[role=tablist],.pack-selector,.color-field,.payment-methods,.notification-sheet,.checkout-dialog") || button.getAttribute("role") === "switch") return;
    const label = (button.innerText || button.getAttribute("aria-label") || "Открыть действие").replace(/\s+/g," ").trim();
